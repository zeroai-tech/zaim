...
export async function GET(req: Request, ctx: { params: Promise<{ action: string }> }) {
  ...
  const accounts = listAccounts(uid).map((a) => ({ id: a.id, label: a.label, email: a.from_email, isDefault: !!a.is_default }))
...
export async function POST(req: Request, ctx: { params: Promise<{ action: string }> }) {
  ...
  if (action === 'register') {
    ...
    if (findUserByEmail(email)) return json({ error: 'An account with that email already exists' }, 409)
    const u = createUser(email, hashPassword(password))
